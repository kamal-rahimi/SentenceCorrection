...
def home():
    ...
    if request.method == 'POST' and form.validate():
        ...
        return render_template('random.html', input=f'<div>{html}</div>')
...
def header(text, color='black'):
    ...
    raw_html = f'<h1 style="margin-top:16px;color: {color}"><center>' + str(text) + '</center></h1>'

    return raw_html

def box(text, gen_text=None):
    """Create an HTML box of text"""
    raw_html = '<div align=left style="padding:8px;font-size:28px;margin-top:15px;margin-bottom:15px;">' + str(
